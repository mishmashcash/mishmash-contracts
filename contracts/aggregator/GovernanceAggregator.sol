...
import "../governance/MishMashGovernance.sol";
...
contract GovernanceAggregator {
  struct Proposal {
    ...
    bool executed;
    bool extended;
    MishMashGovernance.ProposalState state;
  }

  function getAllProposals(MishMashGovernance governance) public view returns (Proposal[] memory proposals) {
  ...
  function getGovernanceBalances(MishMashGovernance governance, address[] calldata accs) public view returns (uint256[] memory amounts) {
  ...
  function getUserData(MishMashGovernance governance, address account)
    public
    view
    returns (
      uint256 balance,
